...
def main():
    ...
    community_root = resolve_apps_root(args.community)
    custom_root = resolve_apps_root(args.custom)
    ...
    def resolve_app_dir(app_name: str):
        # Custom takes precedence
        if custom_root:
            custom_dir = custom_root / app_name
            if custom_dir.is_dir():
                return custom_dir, "custom"

        community_dir = community_root / app_name
        if community_dir.is_dir():
            return community_dir, "community"

        return None, None

    for app_name in allowed_apps:
        app_dir, source = resolve_app_dir(app_name)

        if not app_dir:
            print(f"[WARN] App directory not found in custom or community sources: {app_name}")
            failures.append((app_name, "directory_not_found"))
            if not args.continue_on_error:
                break
            continue

        print(f"[INFO] Using source={source} for app={app_name}")
